...
comprises two-elements \DIFdel{subset} \DIFadd{subsets} of $V$. The elements of each edge
...
vertex set, and \DIFdel{$\{\{u, v\} \in E \mid \{u, v\} \in W\}$}
\DIFadd{$\{\{u, v\} \in E \mid \{u, v\} \subseteq W\}$} as its edge set; that
...
A \emph{clique} is a set of vertices that are \DIFdel{mutually} \DIFadd{pairwise} adjacent, and the
...
\DIFadd{These functions need not be injective.}
...
\DIFadd{Induced subgraphs and isomorphisms on directed graphs preserve the direction of arcs.
Thus an induced subgraph of a directed graph $D = (V, A)$ is a directed graph
that has a subset $W \subseteq V$ as its
vertex set, and $\{(u, v) \in E \mid \{u, v\} \subseteq W\}$ as its edge set;
an isomorphism between directed graphs $D=(V,A)$ and $D'=(V',A')$ is a
bijection $f:V\rightarrow V'$, such that $A' = \{(f(u), f(v)) \mid (u,
v) \in A\}$.}
\Cref{fig:directed-cis-example} shows two directed graphs with a common
induced subgraph highlighted.  \DIFadd{An arrow from $v$ to $w$ represents
the arc $(v,w)$.}
...
\DIFadd{We can also use adjacency matrices and adjacency lists to represent a directed
graph $D=(V,A)$.  In a simple adjacency matrix representation, $A[v][w]$
is $\AlgVar{true}$ if and only if $A$ contains the arc $(v,w)$.  In a simple
adjacency list representation, the list for vertex $v$ contains those vertices $w$ such
that $(v,w) \in A$.  Chapters} \ref{c:mcsplit-i-undirected} \DIFadd{and}
\ref{c:mcsplit-si} \DIFadd{will describe slightly more involved adjacency matrix
and list representations that are used by variants of the \McSplit\ algorithm.}
...
\DIFadd{This model was introduced by} \citet{gilbert1959random}; \DIFadd{see}
\citet{erdos1960on} \DIFadd{and} \citet{moore2011nature} \DIFadd{for properties
of graphs generated using this method.}
...
does $G$ contain a $k$-vertex clique?  If the answer is \DIFdel{``yes'', we}
\DIFadd{``yes'' (i.e.\ if the instance is \emph{satisfiable}) we} can give a proof
...
\begin{itemize}
    \item \textbf{Maximum common induced subgraph \DIFadd{problem} (MCIS).} Find
...
with four vertices.
    \item \textbf{Induced subgraph isomorphism \DIFadd{problem} (ISIP).} Given graphs $G$ (the ``pattern graph'')
...
if so, return the mapping corresponding to an isomorphism from $G$ to an
    ...
    \item \textbf{Minimal induced universal graph \DIFadd{problem}.} Given a family of graphs $\calF$,
...
as an induced subgraph.
    ...
    \Cref{fig:ind-univ-solution} shows a corresponding solution with six vertices.
        ...
        highlighted.  \DIFadd{(It is easy to see that no solution
        with fewer vertices exists for this instance.  Only one graph on fewer than six vertices contains
        $C_5$ as induced subgraph: $C_5$ itself.  This graph contains neither
        $C_3$ nor $C_4$ as an induced subgraph.)}
\end{itemize}
...
\DIFadd{Although Nauty and Traces are very different from the \McSplit\ family
of algorithms that we will introduce in this dissertation, \McSplit\ shares with these
algorithms the idea of partitioning the vertices of a graph, and progressively
refining the partition during search.  Partition backtracking
techniques have also been applied more broadly to problems in computational group theory}
...
induced subgraph isomorphism solver VF2 outperformed Nauty on \DIFadd{the
graph isomorphism problem for} several classes
...
must therefore check during search that no \DIFdel{strongly} connected component of $G$
...
\DIFdel{The Glasgow Subgraph Solver (henceforth Glasgow)
...
\DIFadd{The Glasgow Subgraph Solver}
\citep{DBLP:conf/cp/McCreeshP15,DBLP:conf/gg/McCreeshP020} \DIFadd{(henceforth Glasgow)
reverses this trend towards stronger---and typically also slower---inference
at each search node.
The solver was the fastest for hard induced
subgraph isomorphism instances in a recent experimental evaluation by}
\citet{DBLP:conf/gbrpr/Solnon19}.
\DIFadd{Like Ullmann's algorithm, Glasgow uses bitsets to represent domains and rows
of adjacency matrices.
The Glasgow algorithm aims to achieve most of the domain filtering of LAD and SND with much
less effort per search node.
Rather than achieving generalised arc consistency on the allDifferent
constraint, Glasgow uses a new ``counting'' allDifferent propagator which runs
very fast without giving any consistency guarantee.
In addition, the Glasgow algorithm introduces
\emph{supplemental graphs}---graphs that encode the number of paths
of a given length between pairs of vertices---which provide additional filtering
very cheaply.}

\DIFadd{To describe the supplemental graphs used in the Glasgow Subgraph Solver,
we begin by noting the following easily-verified fact. Let natural numbers
$c$ and $l$, and distinct vertices $v$ and $w$ in the pattern graph, be given.
If there are at least $c$ paths of length $l$ between
$v$ and $w$ in the pattern graph, then there must also be at least $c$ paths of length
$l$ between the corresponding vertices $x_v$ and $x_w$ in the target graph.
This fact allows us to add new constraints to our model that are analogous
to the adjacency constraints in Section~\ref{subsec:cp-model-isip}.  These constraints
can be implemented efficiently by creating supplemental graphs $G'$ and $H'$ for the
pattern and target graphs prior to search, as follows.  The supplemental graphs
have the same vertex sets as the pattern and target graph, and have an edge between
$v$ and $w$ if and only if there at least $c$ paths of length $l$ between $v$ and $w$
in the original graph.  The path-count constraints
that we have described can be propagated by propagating adjacency constraints on the
supplemental graphs.
(The current version of the Glasgow Subgraph Solver
uses only paths of length 2 by default. For more general descriptions of
supplemental graphs and their potential uses in subgraph isomorphism
solvers, see} \cite{DBLP:conf/gg/McCreeshP020,knuth2022art}.)
...
\subsection{Source Code and Problem Instances}

\DIFadd{The repository for this dissertation is available at \url{https://github.com/jamestrimble/phd-thesis}.
This contains new problem instances, along with links to
implementations of the new algorithms.  See the README file in the repository for details.

Some experiments in this dissertation make use of existing sets of benchmark instances.  Source
details are given in the chapter where each set of instances is first used.}
