...
struct Graph readDimacsGraph(char* filename, bool directed, bool edge_labelled, bool vertex_labelled) {
    struct Graph g(0);
    ...
    while (getline(&line, &nchar, f) != -1) {
        if (nchar > 0) {
            ...
            case 'E':
                // labelled edge
                if (sscanf(line, "E %d %d %u", &v, &w, &edge_label)!=3)
                    fail("Error reading a line beginning with E.\n");
                add_edge(g, v-1, w-1, directed, edge_labelled ? edge_label : 1);
                edges_read++;
                break;
            case 'n':
...
struct Graph readGraph(char* filename, char format, bool directed, bool edge_labelled, bool vertex_labelled) {
    struct Graph g(0);
    if (format=='D') g = readDimacsGraph(filename, directed, edge_labelled, vertex_labelled);
