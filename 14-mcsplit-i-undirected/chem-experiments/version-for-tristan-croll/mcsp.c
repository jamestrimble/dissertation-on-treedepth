        ...
        std::vector<std::set<unsigned>> & path_hashes, int & insertion_count)
{
    for (int w : adj_lists[v]) {
        if (!on_path[w]) {
            ...
            hash = 31 * hash + hash_of_int(g.adjmat[v][w]);
        ...
        vector<int> & left, vector<int> & right, unsigned int matching_size_goal)
{
    ...
    for (int i=0; i<=bd.right_len; i++) {
        ...
        solve(g0, g1, adj_lists0, adj_lists1, incumbent, current, new_domains, left, right, matching_size_goal);
        current.pop_back();
    }
    bd.right_len++;
    if (bd.left_len == 0)
        remove_bidomain(domains, bd_idx);
    solve(g0, g1, adj_lists0, adj_lists1, incumbent, current, domains, left, right, matching_size_goal);
}
...
vector<VtxPair> mcs(const Graph & g0, const Graph & g1) {
    ...
    if (arguments.big_first) {
        for (int k=0; k<g0.n; k++) {
            ...
            solve(g0, g1, adj_lists0, adj_lists1, incumbent, current, domains_copy, left_copy, right_copy, goal);
        ...
        vector<VtxPair> current;
        solve(g0, g1, adj_lists0, adj_lists1, incumbent, current, domains, left, right, 1);
    }
