...
vector<unsigned long long> subproblem_times;
...
vector<VtxPair> mcs(const Graph & g0, const Graph & g1) {
    ...
    if (arguments.big_first) {
        for (int k=0; k<g0.n; k++) {
            ...
            subproblem_times.push_back(time_elapsed);
...
int main(int argc, char** argv) {
    ...
    auto total_subproblem_time = 0ull;
    for (auto time : subproblem_times) {
        cout << "Subproblem time:              " << time << endl;
        total_subproblem_time += time;
    }
    auto last_unsat_time = 0ull;
    if (subproblem_times.size() == 1) {
        last_unsat_time = 0ull;
    } else {
        last_unsat_time = subproblem_times[subproblem_times.size() - 2];
    }
    cout << "last_unsat_time:              " << last_unsat_time << endl;
    cout << "sat_time:              " << subproblem_times.back() << endl;
    cout << "total_subproblem_time:              " << total_subproblem_time << endl;
    cout << "total_subproblem_time_minus_last_unsat:              " << total_subproblem_time - last_unsat_time << endl;
