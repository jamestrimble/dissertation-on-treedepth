...
\Cref{sec:mcsplit} introduces the algorithms and its data structures.
...
\Cref{sec:comparison} explains the difference in run times between
...
algorithm.
\Cref{sec:mcsplit-conclusion} concludes.
...
The graph isomorphism algorithm of \citet{DBLP:journals/jacm/SchmidtD76},
...
\citep{Prosser2017McSplitHeuristics}.}
Ties are broken by selecting a label class with the smallest-numbered
vertex of graph $G$.
The $\FuncSty{SelectVertex}$ selects the smallest-numbered vertex
within this label class.

The value-ordering heuristic controls the iteration
order at \lineref{SelectVertex} of \cref{McSplitAlg}. The strategy
...
\Cref{figure:mcis-heatmaps}

Is it better to order the vertices in each graph in ascending or descending
order of degree?  The answer depends on the structure of the two graphs.  To
shed some light on this question, we present results of an experiment comparing
increasing degree and decreasing degree orders for graphs $G$ and $H$ where
both graphs are random $G(n,p)$ graphs.
The $p$ (density) parameter for both graphs
was varied from $0$ to $1$ in steps of $0.01$, for a total of $10201$
pairs of graph.  For each graph, we considered the two possible degree orders;
we call these strategies
``G increasing'' (smallest degree first for graph $G$), ``G decreasing'',
``H increasing'', and ``H decreasing''.
Due to the large number of instances, $n=10$ was used.
Each run was repeated 32 times, and an average (mean) number of calls to the
$\FuncSty{Search}$ function was computed.
...
\begin{figure}[htb]
    ...
    \caption{Heatmaps of \McSplit\ search effort compared to the best strategy, varying pattern density
    (horizontal axis) and target density (vertical axis)}\label{figure:mcis-heatmaps}
\end{figure}

The heatmaps in \Cref{figure:mcis-heatmaps} show how effective each pairing of
variable and value ordering heuristics was best for each pattern density /
target density pair.  To explain the plots, we refer to the first of the four
squares in \Cref{figure:mcis-heatmap} as an example.  On the $x$ and $y$ axes,
we have values of the graph generator's $p$ (density) parameter ranging from 0
to 1 for the pattern and target graph respectively.  The plot shows, for each
pair of density parameters, how the strategy ``G increasing, H increasing''
compares with the best of the four possible strategies.  Dark blue indicates
that ``G increasing, H increasing'' is not beaten by any of the other three
...
between 2 and 10 times the search effort of the best strategy.  From this
subplot we can see that, for random graphs with pattern order 8 and target
order 80, the ``G increasing, H increasing'' strategy appears to be preferable
to the other three strategies if the density of the target graph exceeds that
...
is seldom optimal to use different ordering strategies for graphs $G$ and $H$.
The first and last subplots show that it is best to sort in increasing degree
...
greater than the density of $G$'' often leads to poor results if both graphs are
sparse.
...
complex way.
If we were to pick a simple decision rule for ordering based on the densities
of $G$ and $H$, a plausible candidate would be ``use increasing order for both
graphs if the density of $H$ is greater than $\frac{1}{2}$; otherwise use
decreasing order for both graphs''.  This is the rule we use throughout
this chapter and the next chapter.
The exception is in the comparison of algorithms in \label{sec:mcsplit-experiments},
where we use the results from \citet{DBLP:conf/ijcai/McCreeshPT17}; in these
experiments, decreasing degree order was used throughout.  In practice, this
makes very little difference to the results: none of the directed instances,
and only $4\%$ of the undirected instances, have a graph $G$ with density
...
as the upper bound bound on \lineref{CalcBound} of \Cref{McSplitAlg},
...
in C++ and compiled using g++ 5.3.0. We compare against the
...
(TODO: check I'm saying giving the details in the right place.) This uses
Prosser's MCSa1 algorithm \citep{DBLP:journals/algorithms/Prosser12}, which is
...
\citep{DBLP:journals/cor/SegundoRJ11}. (I also tried using another state of the
art maximum clique solver, IncMC2 \citep{DBLP:journals/informs/LiFJX18}, which
uses a technique from MaxSat solvers to tighten the upper bound during search.
This performed worse overall than MCSa1, and I therefore do not report its
results.)
\item The
...
\subsection{On which Instances Should we Choose \McSplit{$\downarrow$}?}\label{sec:which-mcsplit-down}
