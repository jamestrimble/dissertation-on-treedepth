...
heuristics.
\Cref{sec:mcis-instances} describes the instances used for our experiments.
...
$G$ and $H$ where both graphs are random $G(n,p)$ graphs.  As we will see, the
answer depends in a complex way on the structure of the two graphs.

The $p$ (density) parameter for both graphs was varied from $0$ to $1$ in steps
of $0.01$, for a total of $10201$ pairs of graph.  For each graph, we
considered the two possible degree orders; we call these strategies ``$G$
increasing'' (smallest degree first for graph $G$), ``$G$ decreasing'', ``$H$
increasing'', and ``$H$ decreasing''.  Due to the large number of instances,
$n=10$ was used.  Each run was repeated 32 times with different random graphs,
and an average (mean) number of calls to the $\FuncSty{Search}$ function was
computed across the 32 runs.
...
comparison of algorithms in \Cref{sec:mcsplit-experiments}, we reuse the
...
For each instance, $10 \leq n_G = n_H \leq 100$.  For unlabelled instances, the
