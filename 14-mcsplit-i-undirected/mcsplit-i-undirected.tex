...
\paragraph{Label classes} We require only $\BigO{n_G+n_H}$ space per level of the
...
there are $n_G + n_H$ vertices in the two graphs, at most $n_G + n_H$ label classes can
exist at once, and there are at most $n_G + n_H$ vertices in the union of all of
the $\setG$ and $\setH$ sets. Furthermore, we do not actually need to store the
bits making up a label---we care only that like-labelled vertices are kept
together, so there is no need to store labels explicitly. Nor do we need to
...
henceforth SD, is a backtracking
algorithm that uses a method of partitioning similar to that of \McSplit. Whereas \McSplit\ stores
the vertices of a label class contiguously, SD stores an array containing one integer for
each vertex of $G$, and a similar array for $H$; the array element corresponding to each vertex $v$
stores an identifier for $v$'s label class.  Because SD solves the graph isomorphism
problem, it is unnecessary to calculate an upper bound; the algorithm can backtrack as soon
as the number of vertices of $G$ in a label class differs from the number of vertices of $H$
in that class.  SD takes advantage of the distance matrix (a matrix of shortest path lengths)
for each graph to give a good initial partition and to refine the partition more at each search
node than \McSplit's method based on adjacency matrices. Unfortunately,
the distance matrix cannot be straightforwardly used for the maximum common subgraph problem.
However, it would be possible to adapt \McSplit\ to use SD's array-based representation of
label classes, and the comparison of the two methods would be an interesting avenue for future
work.
...
\begin{figure}[htb]
    ...
    \subfigure[][Unlabelled, undirected] {
        \centering
        \includegraphics*[width=0.92\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/winners10.pdf}
        \label{figure:mcis-heatmap}
    }
    \subfigure[][Unlabelled, directed] {
        \centering
        \includegraphics*[width=0.92\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/winners10directed.pdf}
        \label{figure:sip-heatmap-12}
    }
    \subfigure[][Edge labelled, undirected] {
        \centering
        \includegraphics*[width=0.92\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/winners10edgelabelled.pdf}
