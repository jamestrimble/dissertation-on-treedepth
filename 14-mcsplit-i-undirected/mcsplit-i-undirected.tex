...
\nl \Begin{
\nl \lIf {$|M| = t$}{\KwSty{return} $\AlgVar{true}$} \label{DecisionReturnTrue}
\nl $\AlgVar{bound} \gets |M|  + \sum_{\langle \setG,\setH \rangle \in \AlgVar{future}} \min(|\setG|,|\setH|)$ \label{DecisionCalcBound} \;
\nl \lIf {$\AlgVar{bound} < t$}{\KwSty{return} $\AlgVar{false}$} \label{DecisionPruneSearch}
\medskip
...
\nl   \lIf {$\FuncSty{Search}(\AlgVar{future'},M\cup \{(v,w)\},t)$ \label{DecisionExpandWithV}}{$\KwSty{return}$ $\AlgVar{true}$}
  }
\nl $\setG' \gets \setG \setminus \{v\}$ \label{DecisionRemoveV} \;
\nl $\AlgVar{future} \gets \AlgVar{future} \setminus \{\langle \setG,\setH \rangle\}$\;
\nl \lIf {$\setG' \neq \emptyset$} {$\AlgVar{future} \gets \AlgVar{future} \cup \{\langle \setG',\setH \rangle \}$}
\nl \lIf {$\FuncSty{Search}(\AlgVar{future},M,t)$ \label{DecisionExpandWithoutV}}{$\KwSty{return}$ $\AlgVar{true}$}
}
\;
    ...
    \begin{tikzpicture}
        \foreach \v [count=\i] in {1,2,3,4,5} {
            \foreach \w [count=\j] in {a,b,c,d,e,f} {
                \node[draw,circle,minimum width=.65cm,inner sep=1pt] (\v\w) at (180 - \i*72 - \j*11:3.8) {$\v\w$};
            }
        }
        \draw (1a) -- (3f);
    \end{tikzpicture}   
...
\nl \KwData{Candidate vertices $P$, clique $C$, and target size $t$}
\nl \KwResult{$\AlgVar{true}$ if and only if a clique of size $t$ exists with $C$ as a subset}
\nl \Begin{
\nl \lIf {$|C| = t$}{\KwSty{return} $\AlgVar{true}$} \label{CliqueReturnTrue}
\nl $\AlgVar{bound} \gets |C|  + \FuncSty{ColouringUpperBound}(P)$ \label{CliqueCalcBound} \;
\nl \lIf {$\AlgVar{bound} < t$}{\KwSty{return} $\AlgVar{false}$} \label{CliquePruneSearch}
\medskip
\nl $S \gets $ a subset of $P$ that is an independent set in $\graphA$ \;
\nl \For {$x \in S$ \label{CliqueWLoop}} {
\nl    \lIf {$\FuncSty{Search}(P \cap N(x), C \cup \{x\}, t)$}{\KwSty{return $\AlgVar{true}$}}
     }
\nl  \lIf {$\FuncSty{Search}(P \setminus S, C, t)$}{\KwSty{return $\AlgVar{true}$}}
...
\nl \Begin{
\nl $\KwSty{return}$ $\FuncSty{Search}(V(\graphA),\emptyset, t)$ \label{CliqueFirstExpandCall} \;
}
