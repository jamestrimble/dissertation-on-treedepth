...
\subsubsection{Tie-breaking by vertex degree}

We now turn to the $\FuncSty{SelectVertex}$ procedure and the order over which
vertices in $H$ are iterated in \lineref{WLoop}.  In constraint programming
...
\begin{figure}[h!]
    \centering
    \subfigure[][Undirected graphs] {
        \includegraphics*[width=.9\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/winners10.pdf}
    }
    \subfigure[][Directed graphs] {
        \includegraphics*[width=.9\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/winners10directed.pdf}
    }
...
\begin{figure}[h!]
    \centering
    \subfigure[][Undirected graphs] {
        \includegraphics*[width=0.72\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/bars10.pdf}
    }
    \subfigure[][Directed graphs] {
        \includegraphics*[width=0.72\textwidth]{14-mcsplit-i-undirected/deg_sorting_experiment_fine_grained/observable-figures/bars10directed.pdf}
    }
...
\section{Experimental Evaluation}\label{sec:mcsplit-experiments}
...
\begin{figure}[h!]
    ...
    \subfigure[][Cumulative number of instances solved] {
        \centering
        \includegraphics*[width=0.75\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/paper/gen-graph-plain-cumulative.pdf}
    ...
    \subfigure[][\McSplit\ versus \McSplit{$\downarrow$}, with one plotted point per instance.
        ...
        \centering
        \includegraphics*[width=0.6\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplit-vs-mcsplitdown}
...
\begin{figure}[h!]
    ...
    \subfigure[][Cumulative number of instances solved] {
        \centering
        \includegraphics*[width=0.75\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/paper/gen-graph-33ved-cumulative.pdf}
    ...
    \subfigure[][\McSplit\ versus \McSplit{$\downarrow$}, with one plotted point per instance.
            ...
            many vertices as each of the input graphs.] {
        \centering
        \includegraphics*[width=0.6\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplit-vs-mcsplitdown}
...
TODO: write about \McSplitDown. Probably move this to its own section
...
\Cref{figure:decision-scatter-last-two-all-others} compares, for completeness, the
...
\begin{figure}[h!]
    ...
    \subfigure[][Unlabelled, undirected, connected] {
        \centering
        \includegraphics*[width=0.75\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/paper/gen-graph-plain-connected-cumulative.pdf}
        \label{figure:plain-connected-cumulative}
    }
    \par\bigskip
    \subfigure[][Vertex and edge labelled, undirected, connected] {
        \centering
        \includegraphics*[width=0.75\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/paper/gen-graph-33ve-connected-cumulative.pdf}
...
\section{On which Instances Should we Choose \McSplit{$\downarrow$}?}\label{sec:which-mcsplit-down}
...
all maximum common subgraphs of a pair of graphs, rather than just a single subgraph.
...
Krissinel and Henrick \cite{DBLP:journals/spe/KrissinelH04} introduce a forward-checking
algorithm for finding all maximum common subgraphs of two given graphs.  Their algorithm
...
may still be mapped to at least one vertex in the second graph.

We can emulate the search tree of Krissinel and Henrick's algorithm with McSplit
by making two modifications in addition to the modifications for finding all maximum
common subgraphs described in this section's first paragraph.  First, rather 
than using $|M|  + \sum_{\langle \setG,\setH \rangle \in \AlgVar{future}} \min(|\setG|,|\setH|)$
as the bound on \lineref{CalcBound} of \Cref{McSplitAlg},
we use $|M|  + \sum_{\langle \setG,\setH \rangle \in \AlgVar{future}} |\setG|$.  Clearly,
the bound computed in this way is weaker than or equal to McSplit's bound.  The second
change is to modify the variable selection heuristic so that it chooses a label class with
as small an $H$ set as possible, rather than minimising $\max(|\setG|,|\setH|)$.

We ran a small experiment to compare \McSplit\ with the algorithm of Krissinel and Henrick (KH).
We generated nine random $G(n,p)$ graphs with 18 vertices and values of $p$ in the set
$\{0.1, 0.2, \dots, 0.9\}$.  For each pair of graphs --- a total of 81 graph pairs ---
we ran three algorithms: KH, McSplit, and the version of version
of McSplit described in the previous paragraph with modifications to mimic
KH.  In each case, the problem was to find all maximum common subgraphs.
The first plot in \Cref{fig:mcsplit-vs-kh} compares the run time of McSplit with that
of KH, with one point per instance.  The mean run time of McSplit
was 25 ms per instance, compared to 659 ms for KH.  On the most
challenging instances, McSplit was around two orders of magnitude faster than KH.
KH was faster than McSplit on only two instances.

\begin{figure}[h!]
    \centering
    \subfigure[][Krissinel and Henrick (KH) vs McSplit] {
        \centering
        \includegraphics*[width=0.4\textwidth]{14-mcsplit-i-undirected/krissinel/little-experiment/plots/kh-vs-mcsplit}
        \label{figure:plain-cumulative}
    }\subfigure[][KH vs modified McSplit] {
        \centering
        \includegraphics*[width=0.4\textwidth]{14-mcsplit-i-undirected/krissinel/little-experiment/plots/kh-vs-mcsplit-prime}
        \label{figure:plain-scatter}
    }
    \caption{Run times (ms) for McSplit, modified McSplit, and the algorithm of
        Krissinel and Henrick \cite{DBLP:journals/spe/KrissinelH04} on a set
        of random 18-vertex graphs, for the problem of finding all maximum
        common subgraphs.}\label{fig:mcsplit-vs-kh}
\end{figure}

The second plot of \Cref{fig:mcsplit-vs-kh} compares the run time of the modified
McSplit --- which aims to imitate KH --- with that of KH on each instance.  On most
instances, the algorithms had very similar run times, lending support to our claim
that the algorithm function similarly.  Interestingly, KH was slightly faster than
McSplit on most of these instances, suggesting that --- at least on these small,
random graphs --- the benefits of McSplit come from the superior bounding function
and variable selection heuristic made possible by its data structure, rather than
from the speed of the partitioning method of domain filtering.  The modified version
of McSplit took 525 ms per instance on average.
