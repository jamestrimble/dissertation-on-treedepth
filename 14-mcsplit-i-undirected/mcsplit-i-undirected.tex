...
\chaptermark{Maximum Common Induced Subgraph}
...
strictly greater than --- the size of the \DIFdel{incumbent.} \DIFadd{incumbent (the best solution found so far).} Second, on \lineref{PruneSearch},
...
set of candidate vertices for addition to the $C$; \DIFdel{we require that $C \cap P = \emptyset$
and that each vertex in $P$ is adjacent in $\graphA$ to every member of $C$.}  \DIFadd{for each
vertex $v$ in $P$, it holds that $v$ is not in $C$ but is adjacent to every element of $C$.} The
...
(\cref{CliqueDecisionAlg}), where \DIFdel{$P = \{G \times H \mid \langle G,H \rangle \in
\AlgVar{future}\}$ and $C = M$.} \DIFadd{$C = M$ and $P = \bigcup_{\langle \setG,\setH \rangle \in \AlgVar{future}} \setG \times \setH$.}
\DIFdel{That is, each call to the
...
\DIFadd{That is, each call to the
$\FuncSty{CliqueSearch}()$ function takes as its argument $C$ every node
$(v,w)$ in the association graph corresponding to an assignment in $M$, and
as its argument $P$ a set containing
every node $(v,w)$ in the association graph such that $v$ and $w$ are in the
same label class in $\AlgVar{future}$.  As an example of the $P$ set, consider
the value of $\AlgVar{future}$ shown in} \Cref{fig:alg3},
\DIFadd{$\{\langle \{2,3\},\{d,f\} \rangle\, \langle \{4,5\}, \{b,c,e\} \rangle\}$,
where, for example, vertices 2 and 3 of graph $G$ may each be mapped to either vertex
$d$ or vertex $f$ of graph $H$.  In this case,
$P=\{(2,d), (2,f), (3,d), (3,f), (4,b), (4,c), (4,e), (5,b), (5,c), (5,e), (6,b), (6,c), (6,e)\}$.}
...
\DIFdel{$P = \{G \times H \mid \langle G,H \rangle \in \AlgVar{future}\}$}
\DIFadd{$P = \bigcup_{\langle \setG,\setH \rangle \in \AlgVar{future}} \setG \times \setH$}
in the association graph.  For each
\DIFdel{$\langle G, H\rangle$}
...
\DIFdel{$\{(v,w) \mid w \in H\}$}
\DIFadd{$\{(v,w) \mid w \in \setH\}$}
for each
\DIFdel{$v \in G$}
\DIFadd{$v \in \setG$}.
If
...
\DIFdel{$\{(v,w) \mid v \in G\}$}
\DIFadd{$\{(v,w) \mid v \in \setG\}$}
for each
\DIFdel{$w \in H$}
\DIFadd{$w \in \setH$}.
...
\DIFdel{$S = \{(v,w) \mid w \in H\}$}
\DIFadd{$S = \{(v,w) \in P \mid w \in \setH\}$}, with
\DIFdel{$H$}
\DIFadd{$\setH$}
and $v$ taking the values selected by $\FuncSty{SelectLabelClass}(\AlgVar{future})$
and
\DIFdel{$\FuncSty{SelectVertex}(G)$}
\DIFadd{$\FuncSty{SelectVertex}(\setG)$}
on \DIFdel{lines} \lineref{DecisionSelectClass} and \lineref{DecisionSelectVertex}
...
\]
    ...
    \DIFdel{$P \cap N_A((v,w)) = \{G \times H \mid \langle G,H \rangle \in \AlgVar{future'}\}$}
    \DIFadd{$P \cap N_A((v,w)) = \bigcup_{\langle \setG,\setH \rangle \in \AlgVar{future'}} \setG \times \setH$}
    ...
    there exists
    a label class
    \DIFdel{$\langle G',H' \rangle \in \AlgVar{future}$}
    \DIFadd{$\langle \setG',\setH' \rangle \in \AlgVar{future}$}
    such that
    \DIFdel{$v \in G'$}
    \DIFadd{$v \in \setG'$}
    and
    ...
    element of
    \DIFdel{$\{G \times H \mid \langle G,H \rangle \in \AlgVar{future'}\}$,}
    \DIFadd{$\bigcup_{\langle \setG,\setH \rangle \in \AlgVar{future'}} \setG \times \setH$,}
    as required.
    ...
    \DIFdel{$P \setminus S = \{G \times H \mid \langle G,H \rangle \in \AlgVar{future'}\}$.}
    \DIFadd{$P \setminus S = \bigcup_{\langle \setG,\setH \rangle \in \AlgVar{future'}} \setG \times \setH$.}
    We have
    \DIFdel{Note: 3.1 to 3.4 have been corrected}
\begin{gather}
    P \setminus S = \\
...
finding all maximum common subgraphs described in \Cref{sec:extensions}.
...
\DIFadd{(Recall from} \Cref{subsec:plot-conventions} \DIFadd{that the grey bands represent timeouts.)}
...
unlabelled, \DIFdel{undirected, unconnected instances} \DIFadd{undirected graphs,
and does not support the connected variant of the problem}).
\end{itemize}
...
\begin{figure}[htb]
	...
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-cpfc-random}
        \label{figure:mcsplain-runtime-mcsplitdown-cpfc-random}
    }\subfigure[][Regular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-cpfc-regular-mesh}
        \label{figure:mcsplain-runtime-mcsplitdown-cpfc-regular-mesh}
    }\subfigure[][Irregular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-cpfc-irregular-mesh}
        \label{figure:mcsplain-runtime-mcsplitdown-cpfc-irregular mesh}
    }
    \subfigure[][Bounded valence graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-cpfc-bv}
        \label{figure:mcsplain-runtime-mcsplitdown-cpfc-bv}
    }
    \subfigure[][Irregular BV graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-cpfc-irregular-bv}
...
\begin{figure}[htb]
    ...
    \subfigure[][Random graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-kdown-random}
        \label{figure:mcsplain-runtime-mcsplitdown-kdown-random}
    }\subfigure[][Regular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-kdown-regular-mesh}
        \label{figure:mcsplain-runtime-mcsplitdown-kdown-regular-mesh}
    }\subfigure[][Irregular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-kdown-irregular-mesh}
        \label{figure:mcsplain-runtime-mcsplitdown-kdown-irregular mesh}
    }
    \subfigure[][Bounded valence graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-kdown-bv}
        \label{figure:mcsplain-runtime-mcsplitdown-kdown-bv}
    }
    \subfigure[][Irregular BV graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-kdown-irregular-bv}
...
\begin{figure}[htb]
    ...
    \subfigure[][Random graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-clique-random}
        \label{figure:mcsplain-runtime-mcsplitdown-clique-random}
    }\subfigure[][Regular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-clique-regular-mesh}
        \label{figure:mcsplain-runtime-mcsplitdown-clique-regular-mesh}
    }\subfigure[][Irregular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-clique-irregular-mesh}
        \label{figure:mcsplain-runtime-mcsplitdown-clique-irregular mesh}
    }
    \subfigure[][Bounded valence graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-clique-bv}
        \label{figure:mcsplain-runtime-mcsplitdown-clique-bv}
    }
    \subfigure[][Irregular BV graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcsplain-runtime-mcsplitdown-clique-irregular-bv}
...
\begin{figure}[htb]
    ...
    \subfigure[][Random graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-cpfc-random}
        \label{figure:mcs33ved-runtime-mcsplitdown-cpfc-random}
    }\subfigure[][Regular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-cpfc-regular-mesh}
        \label{figure:mcs33ved-runtime-mcsplitdown-cpfc-regular-mesh}
    }\subfigure[][Irregular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-cpfc-irregular-mesh}
        \label{figure:mcs33ved-runtime-mcsplitdown-cpfc-irregular mesh}
    }
    \subfigure[][Bounded valence graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-cpfc-bv}
        \label{figure:mcs33ved-runtime-mcsplitdown-cpfc-bv}
    }
    \subfigure[][Irregular BV graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-cpfc-irregular-bv}
...
\begin{figure}[htb]
    ...
    \subfigure[][Random graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-clique-random}
        \label{figure:mcs33ved-runtime-mcsplitdown-clique-random}
    }\subfigure[][Regular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-clique-regular-mesh}
        \label{figure:mcs33ved-runtime-mcsplitdown-clique-regular-mesh}
    }\subfigure[][Irregular meshes] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-clique-irregular-mesh}
        \label{figure:mcs33ved-runtime-mcsplitdown-clique-irregular mesh}
    }
    \subfigure[][Bounded valence graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-clique-bv}
        \label{figure:mcs33ved-runtime-mcsplitdown-clique-bv}
    }
    \subfigure[][Irregular BV graphs] {
	\centering
        \includegraphics*[trim=0 0.05cm 0 0,clip,width=0.320001\textwidth]{14-mcsplit-i-undirected/plots-based-on-ijcai-paper/more-analysis/plots/mcs33ved-runtime-mcsplitdown-clique-irregular-bv}
...
\begin{figure*}[htb]
    ...
    \subfigure[][Labelled, directed instances] {
...
We now turn to the extreme case in which \DIFdel{all vertex labels are distinct
in each of the two input graphs.} \DIFadd{each vertex label appears exactly
once in graph $G$ and exactly once in graph $H$.}\footnote{\DIFadd{Even
with this restriction, the maximum common induced subgraph problem
remains \NP-hard.  To see this, consider the \NP-hard problem of finding
a clique of maximum size in an unlabelled, undirected graph $G$.  Let $H$
be the complete graph $K_{|V(G)|}$ with the same vertex set as $G$.
Label the vertices of both graphs with the integers from 1 to $|V(G)|$.
A maximum common subgraph between graphs $G$ and $H$ using this vertex labelling
corresponds to a maximum clique in the unlabelled graph $G$.}}
\Cref{fig:clique-bound-labelled} shows
...
previous state of the art for unlabelled, undirected instances.  \DIFdel{For
instances with many distinct vertex and edge labels, however, the MCSa1 maximum clique
algorithm remains the fastest solver.}
...
\DIFadd{Although this chapter has not focused on the systematic
selection of the best algorithm for a given instance (on this topic,
see} \citet{dilkas2018}), \DIFadd{we can draw the following broad conclusions.
For unlabelled graphs, \McSplit\ and \McSplitDown\ are generally faster
than a maximum clique solver; the opposite is true for graphs with many distinct
vertex and edge labels.  For families of instances where a maximum common induced
subgraph is almost as large as the input graphs, \McSplitDown\ is often dramatically
faster than \McSplit; for families with small maximum common subgraphs,
\McSplit\ is the fastest algorithm overall.}

The next chapter introduces new variants of \McSplit\ that explore the effects
of swapping graphs $G$ and $H$.  
\Cref{sec:mcsplit-sparse} will return once more to maximum common induced subgraph,
