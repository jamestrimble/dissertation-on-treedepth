...
The implementation of \McSplit-SI does not yet support edge labels, but it
would be possible to do so with a fairly simple modification to the algorithm.
Moreover, this modification has the nice property that it does not change the
...
the adjacency lists.  When partitioning is carried out, this leaves the
vertices in each new label class sorted by edge label.  By simultaneously
