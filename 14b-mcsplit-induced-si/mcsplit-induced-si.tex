...
\section{\McSplit-SI achieves GAC}
...
search node.  However, the data structure maintained by these algorithms are
much more lightweight than those of Glasgow.  VF3 and RI do not maintain
a domain for each vertex in the pattern graph; they simply maintain a partition
of the vertices in the each graph into three sets: ($A$) vertices that have been
mapped already, ($B$) unmapped vertices that are adjacent to at least one mapped vertex, and
($C$) all other unmapped vertices.
If the $B$ (resp. $C$) set of the pattern graph is smaller than the $B$ (resp. $C$)
set of the target graph, the algorithm can backtrack.
This data structure can be updated with the same
time complexity as the partitioning step of \McSplit-SI (and presumably with a slightly
...
same parameters.  We made this choice for two reasons: first, because of the size
