...
on three sets of benchmark instances.  The first set of instances is a large,
existing set of unlabelled graph pairs where we solve the decision problem.
The second set of instances is a new set of decision-problem
instances recently proposed by Donald Knuth.
The final set of instances is based on another existing benchmark set;
we solve the problem of counting all induced subgraph isomorphisms from the pattern to the target.
In this set of instances, all graphs are directed and some have labelled
vertices.
...
\begin{itemize}
    ...
    \item \textbf{Phase:} Pairs of Erd\H{o}s-Rényi $G(n,p)$ random graphs
        with parameters chosen near the satisfiable/unsatifiable phase transition.
