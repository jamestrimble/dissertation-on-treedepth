...
\centering
\footnotesize
    \begin{tabular}{p{0.1\linewidth} p{0.2\textwidth} p{0.1\linewidth} p{0.1\linewidth} p{0.1\linewidth} p{0.18\linewidth}}
 \toprule
     Family & Description & Count & Target $n$ & Target density & Source\\ [0.5ex]
 \midrule
    1 & Randomly generated scale-free graphs \cite{DBLP:books/daglib/0012457}: 80
        satisfiable instances and 20 unsatisfiable instances.  In each instance,
        $|V(G)| = 0.9 \times |V(H)|$.
        & 100 & \numrange{200}{1000} & w & \citet{DBLP:journals/constraints/ZampelliDS10} \\
     \rule{0pt}{2.3ex}Pairs of graphs derived from the Stanford GraphBase \cite{DBLP:books/daglib/0071477}.
     	TODO NOTE TO SELF: have a look at Cortadella and Valiente 1999 A relational view of SI
     	& x & y & z & w & v \\
 \bottomrule
\end{tabular}
\caption{TODO instance families}
\label{tab:TODO}
\end{table}
...
on which a combination of Glasgow's restarts and stronger filtering methods results in
...
\caption{The number of instances solved by each solver for each family of instances.  The second
...
instance, with a separate sub-figure for each instance family.  On the PR, Phase, Meshes,
...
On an instance-by-instance basis, does the best \McSplit-SI variant typically run faster than
the best of the other solvers? To answer this question, consider two virtual solvers:
VBM (virtual best \McSplit-SI) which takes the fastest run time of \McSplit-SI, \McSplit-SI-AM,
and \McSplit-SI-static, and VBO (virtual best other solver), which takes the fastest run time
of the five non-\McSplit\ solvers: Glasgow, Glasgow without supplemental graphs, RI, RI-DS, and VF3.
Each of these virtual solvers is equivalent to running the constitutent solvers in parallel
and stopping when the first solver terminates; alternatively, we can view a virtual solver
as an idealised portfolio solver with an oracle that can
to select the best constituent solver for a given instance.

\Cref{figure:mcsplitsi-vbs-scatter-by-family} shows the run times of VBM (horizontal axis)
and VBO (vertical axis) for each instance.  On all families except Scalefree, the number
of instances where VBM is faster than VBO is greater than the number of instances where
the reverse is true. 
The overall patterns in this picture are similar to those in
\Cref{figure:mcsplitsi-scatter-by-family}, which compared only \McSplit-SI and Glasgow.
There are two notable differences.  First, RI and VF3 outperform Glasgow on some of the easier
families such as Images, moving points closer to the $x=y$ line in
\Cref{figure:mcsplitsi-vbs-scatter-by-family} than in
\Cref{figure:mcsplitsi-scatter-by-family}.  Second, \McSplit-SI-static performs well
on many of the instances where Glasgow is much faster than \McSplit-SI; as a result,
there are few instances where VBO is much faster than VBM.

\begin{figure}[h!]
    \centering
    \subfigure[][Scalefree] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-Scalefree-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \subfigure[][LV] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-LV-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \subfigure[][BVG] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-BVG-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }

    \subfigure[][M4D] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-M4D-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \subfigure[][Rand] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-Rand-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \subfigure[][PR] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-PR-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }

    \subfigure[][Phase] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-Phase-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \subfigure[][Meshes] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-Meshes-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \subfigure[][Images] {
        \includegraphics*[width=0.22\textwidth]{14b-mcsplit-induced-si/decision-instances-experiment/experiment/plots/by-family/scatter-Images-vbmcsplit-vbos.pdf}
        \label{figure:TODO}
    }
    \caption{Run times in ms of virtual best \McSplit-SI variant (horizontal axis) and virtual best other solver (vertical axis), by family.}
