...
graphs.  For harder instances (say, those that cannot be solved in less than 10 seconds by
any of the used solvers), \McSplit-SI does not dominate, but it remains competitive with
Glasgow, the leading existing solver.  In particular, a version of \McSplit with a presolve
step solves more instances within 1000 seconds than does Glasgow.}

\DIFadd{A limitation of the set of benchmark instances that we have used is that most of the
instances are trivial for the best solvers, and the results of our experiments
