...
void solve(const Graph & g0, const Graph & g1, vector<VtxPair> & incumbent,
        ...
        int parent_node, int decision_a, int decision_b)
{
    ...
    bool is_root = nodes == 1;

    bool updated_incumbent = false;
    if (current.size() > incumbent.size()) {
        updated_incumbent = true;
        incumbent = current;
    ...
    if (arguments.big_first && matching_size_goal == 5)
        std::cout << "NODE " << this_node << " " << parent_node << " " << bound <<
            " " << decision_a << " " << decision_b << std::endl;
    if (!arguments.big_first)
        std::cout << "NODE " << this_node << " " << parent_node << " " << bound <<
            " " << decision_a << " " << decision_b << " " << (int) updated_incumbent << std::endl;
    ...
    for (int i=0; i<=bd.right_len; i++) {
        ...
        solve(g0, g1, incumbent, current, new_domains, left, right, matching_size_goal, this_node,
                v, w);
        int nodes_in_subtree = nodes - nodes_before;
        if (is_root)
            std::cout << "st_nodes " << nodes_in_subtree << std::endl;
        current.pop_back();
    }
    bd.right_len++;
    if (bd.left_len == 0)
    ...
    int nodes_before = nodes;
    solve(g0, g1, incumbent, current, domains, left, right, matching_size_goal, this_node, v, -1);
    int nodes_in_subtree = nodes - nodes_before;
    if (is_root)
        std::cout << "st_nodes " << nodes_in_subtree << std::endl;
}
...
vector<VtxPair> mcs(const Graph & g0, const Graph & g1) {
    ...
    if (arguments.big_first) {
        for (int k=0; k<g0.n; k++) {
            ...
            nodes = 0;
            solve(g0, g1, incumbent, current, domains_copy, left_copy, right_copy, goal, 0, -1, -1);
        ...
        vector<VtxPair> current;
        solve(g0, g1, incumbent, current, domains, left, right, 1, 0, -1, -1);
    }
