...
it can be useful to swap the two input graphs when calling a maximum common subgraph
...
chapter is to introduce \McSplit2, a modified
...
\Cref{sec:when-swap} presents initial results on the effect of swapping graphs,
and introduces two heuristics for deciding when we should swap.
\Cref{sec:explaining-sd-so} gives reasons for the success of the swapping
rules.
\Cref{sec:mcsplit2} introduces \McSplit2, a version of \McSplit\ that can branch
on two sides.
...
shows run times for the MCS Plain instances.  The horizontal axis shows run time in milliseconds
...
\section{\McSplit2: Generalising \McSplit-SD and \McSplit-SO}\label{sec:mcsplit2}
...
\nl        $\setG'' \gets \setG' \cap \N(\graphG, v)$ \label{NewPWithEdgeMS2} \;
...
In addition to \McSplit2, we implemented the reverse heuristic, which replaces the condition
$|G| \leq |H|$ on \Cref{ChooseBranchingSideMS2} with $|G| > |H|$.  We call this \McSplit2'.
...
\centering
 ...
 \midrule
    ...
    \rule{0pt}{2.3ex}\McSplit2 & \McSplit\ with branching on both sides & \Cref{McSplit2Alg} \\
    \rule{0pt}{2.3ex}\McSplit2' & The same as \McSplit2, but with the condition for branching on a vertex of $H$ rather than
...
\Cref{figure:mcsplit2-cumulative-random2} shows a cumulative plot comparing \McSplit, \McSplit2, \McSplit2',
and \McSplit-SD on our family of 24-vertex graphs with varying density.  \McSplit-SD is fastest of
the four algorithms, followed by \McSplit2.

\Cref{figure:mcsplit2-cumulative-random3} compares \McSplit, \McSplit2, \McSplit2',
and \McSplit-SO on our family of graphs with similar density but varying vertex count.
\McSplit-SO and \McSplit2 have almost identical results.

\Cref{figure:mcsplit2-cumulative-random4} introduces a new set of random instances, with both
...
$[0,1]$.  Of the five algorithms, \McSplit-SD is the overall winner.  \McSplit2
...
\Cref{figure:mcsplit2-cumulative-mixandmatch} shows results for another new family of instances,
...
On these ``mix and match'' instances, \McSplit-SD, \McSplit-SO, and \McSplit2 have similar performance, and
...
%Overall, these results may be viewed as a partial success story for \McSplit2.  For each of the four families
%of graphs, \McSplit2 is faster overall than \McSplit, and its performance is not far from that of the better
%of \McSplit-SD and \McSplit-SO.  However, it may be viewed as slightly disappointing that \McSplit2 is not
...
\begin{figure}[htb]
    ...
    %}
    \caption{Cumulative plots comparing \McSplit, \McSplit2, and the swapping versions of \McSplit\ on four graph classes.}
    \label{figure:mcsplit2-cumulative}
\end{figure}

On all four instance families shown in \Cref{figure:mcsplit2-cumulative}, \McSplit2'
is the worst-performing algorithm.  This is surprising in one sense, as \McSplit2' is the algorithm
of the five used that comes closest to a smallest-domain-first heuristic.  However, the poor performance
of \McSplit2' is explained by our foregoing discussion of the effect of branching side on upper
bounds.

\subsection{Graph pairs with equal density and order}

In the previous section, we saw that \McSplit2 is substantially faster overall than \McSplit\ for families
of instances in which the pattern and target graph differ in density or order.  Yet the results in that
section could be viewed as somewhat disappointing; \McSplit2 is outperformed by \McSplit-SD on instances
whose graphs have different densities, and by \McSplit-SO on instances whose graphs have similar density
but different numbers of vertices.  Are there families of instances for which \McSplit2 clearly outperforms
...
\McSplit.  Thus, if \McSplit2 outperforms \McSplit on any of these families, it also
...
\Cref{figure:mcsplit-vs-mcsplit2-gnm} shows a cumulative plot and a scatter plot of run times of \McSplit\ and \McSplit2.
The cumulative plot also shows results for \McSplit2'.  The results are somewhat disappointing; all three algorithms
have very similar performance overall.  The scatter plot shows that \McSplit2 is often slightly faster than \McSplit\ --- indeed,
\McSplit2 is faster than \McSplit\ on 174 instances and slower than \McSplit\ on only 91 instances --- but the differences
...
\begin{figure}[htb]
	...
	\centering
	\includegraphics*[width=0.41\textwidth]{14-mcsplit-i-undirected/modified-mcsplit-experiment/plots/plots/mcsplit-vs-mcsplit2-gnm}
        \label{figure:mcsplit-vs-mcsplit2-gnm-scatter}
    }
    \caption{Comparison of \McSplit\ and \McSplit2 on $G(n,m)$ graphs}
    \label{figure:mcsplit-vs-mcsplit2-gnm}
\end{figure}

Our second instance generator creates two graphs using the Barabási-Albert model \cite{barabasi1999emergence}.
This model generates graphs with a wide distribution of degrees.  It has two parameters, $n$ and $m$, such that
$1 \leq m < n$.  The generator works as follows: initially, a graph on $m$ vertices with no edges is created.
...
added.  We use NetworkX implementation of this generator \cite{networkx}.  For each instance, $n$ is chosen at random
...
\Cref{figure:mcsplit-vs-mcsplit2-ba} compares \McSplit\ with \McSplit2 on these Barabási-Albert instances.
Overall, \McSplit2 has a small speed advantage over \McSplit.  The scatter plot shows that the two algorithms
tend to have very similar run times, with \McSplit2 the slightly faster algorithm in most cases.
...
\begin{figure}[htb]
    ...
    \subfigure[][Scatter plot with one point per instance] {
	\centering
	\includegraphics*[width=0.41\textwidth]{14-mcsplit-i-undirected/modified-mcsplit-experiment/plots/plots/mcsplit-vs-mcsplit2-ba}
        \label{figure:mcsplit-vs-mcsplit2-ba-scatter}
    }
    \caption{Comparison of \McSplit\ and \McSplit2 on Barabasi-Albert graphs}
...
shows the results of running our algorithms on these instances.  The cumulative curve shows that \McSplit2 is, by a small margin, the
fastest algorithm overall, and that \McSplit2' is the slowest.  \McSplit2 and \McSplit\ exceeded
the time limit on 105 and 127 instances respectively.  The scatter plot shows that the two
algorithms often have very different run times, with \McSplit\ outperforming \McSplit2 on several instances.
...
\begin{figure}[htb]
    ...
    \subfigure[][Scatter plot with one point per instance] {
	\centering
	\includegraphics*[width=0.41\textwidth]{14-mcsplit-i-undirected/modified-mcsplit-experiment/plots/plots/mcsplit-vs-mcsplit2-ba-gnm}
        \label{figure:mcsplit-vs-mcsplit2-ba-gnm-scatter}
    }
    \caption{Comparison of \McSplit\ and \McSplit2 on $G(n,m)$ / Barabasi-Albert graphs pairs}
    \label{figure:mcsplit-vs-mcsplit2-ba-gnm}
\end{figure}

To summarise, this section has shown a very modest win for \McSplit2; for our two families of
instances using the Barabasi-Albert model, \McSplit2 slightly outperforms \McSplit\ --- and therefore
...
as \McSplit2 does.  However, it is common in constraint programming to combine multiple models
...
introduced \McSplit2, a version of \McSplit\ that branches on vertices of both graphs.  We have
...
There are many potential avenues for future work.  Most obviously, we could investigate
...
With \McSplit2, we have only begun to explore the possibilities of branching on both graphs.
...
combine the two-sided branching of \McSplit2 with the initial swapping rules of \McSplit-SD
or \McSplit-SO.
