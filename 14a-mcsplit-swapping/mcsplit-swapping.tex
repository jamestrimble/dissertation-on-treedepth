...
\centering
 ...
 \midrule
    ...
    \McSplit-SD & \McSplit\ with $G$ and $H$ swapped if and only if
    		$\left|\frac{1}{2} - d_G\right| > \left|\frac{1}{2} - d_H\right|$ & \Cref{McSplitSDAlg} \\
    \McSplit-SO & \McSplit\ with $G$ and $H$ swapped if and only if $n_G > n_H$ & \Cref{McSplitSOAlg} \\
    \McSplit2 & \McSplit\ with branching on both sides & \Cref{McSplit2Alg} \\
    \McSplit2' & The same as \McSplit2, but with the condition for branching on a vertex of $H$ rather than
...
$[0,1]$.  Of the five algorithms, \McSplit-SD is the overall winner.  \McSplit-SO and \McSplit2
appear to be slightly faster than \McSplit, particularly for the easiest instances, but
...
instance.  On the instances, \McSplit-SD, \McSplit-SO, and \McSplit2 have similar performance, and
all are clearly faster overall than \McSplit.
TODO: say how often \McSplit-SD and \McSplit-SO agree.
...
where no rationale of the ordering of graphs is given.
Hoffmann, McCreesh and Reilly \cite{UpcomingAAAIPaper} put the smaller graph first
in the $k\downarrow$ algorithm with no explanation; the presumable reason is that $k\downarrow$
...
graph first in FMCS.\footnote{More precisely, the algorithm chooses a graph whose largest
connected component is as small as possible, as FMCS searches only for connected subgraphs.}
A short explanation is given in a code comment \cite{DalkeFmcsSource}: this strategy tends to
...
I have not found any prior algorithm on maximum common subgraph that branches on both graphs,
...
reversed \cite{DBLP:conf/ecai/Geelen92} --- without any additional overhead in memory use
or execution time.
...
We have shown that both rules are effective in reducing the run time of \McSplit\ on random
graphs.  Using insights from the behaviour of these swapping versions of \McSplit, we have
introduced \McSplit2, a version of \McSplit\ that branches on vertices of both graphs.  We have
shown that this is an improvement on \McSplit, but we have not succeeded in finding graph
families for which it clearly outperforms the swapping rules.

Future work could explore swapping rules that take both density and order into account,
